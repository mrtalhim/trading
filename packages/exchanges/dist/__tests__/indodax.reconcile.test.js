...
import { reconcile } from '../indodax/reconcile.js';
...
function balance(asset, free, total) {
...
describe('reconcile (ownership + startup/periodic)', () => {
    ...
    it('detects fills of owned orders that closed on the exchange', () => {
        ...
        const result = reconcile(input({ recordedOrders: [recorded], openOrders: [filled], balances: [balance('btc', 1, 1)] }));
    ...
    it('derives position and quoteFree from balances', () => {
        const result = reconcile(input({ balances: [balance('idr', 3_000_000, 3_000_000), balance('btc', 0.5, 2)] }));
...
//# sourceMappingURL=indodax.reconcile.test.js.map