...
- **Verdict (recorded 2026-08-10)**: the 2026 conclusions **replicate in a ~50%-higher-volume contrast regime**. The sweep/baseline findings are regime-robust, not artifacts of one window — no architecture change, and the M9/M10 effort concentration on the risk/exit layer is reaffirmed. Also flagged (pre-existing, still unfixed): `validRate` mislabels hold-with-confidence rows as non-hold (17–62% real action rates), and both regimes share the same slice-size caveat (2×~26-day windows per asset).
