...
import { IndodaxPublicApiClient } from '@trading/exchanges';
import { loadDecisions } from '@trading/backtest';
import { AgentEngine } from './engine.js';
import { statusPath, writeCommand } from './signal.js';
...
function num(opt, key, fallback) {
    return opt[key] === undefined ? fallback : Number(opt[key]);
}
async function syntheticDecisions(candles, sampleEvery) {
    const decisions = [];
...
async function pullDataset(opt) {
    ...
    const sorted = [...bars].sort((a, b) => a.timestamp - b.timestamp).slice(-candles);
...
async function runAgent(opt) {
    const datasetDir = opt.dataset ?? DEFAULT_DATASET;
    const dataset = new JsonlLoader(datasetDir);
    const pins = [];
    for await (const c of dataset.candles())
        pins.push(c);
    const decisions = opt.decisions !== undefined
    ...
    process.stdout.write(`ran ${result.candleCount} candles, ${result.tradeCount} trades, ` +
        `realizedPnl ${result.realizedPnl.toFixed(2)} IDR, checksum ${result.checksum}\n`);
}
async function sendCommand(opt, command) {
...
export async function runAgentCli(argv = process.argv.slice(2)) {
    ...
    switch (command) {
        ...
        case 'status':
            ...
            break;
        default:
            throw new Error(`unknown command '${command}' (expected run|pull|pause|resume|shutdown|status)`);
    }
}
